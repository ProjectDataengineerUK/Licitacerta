terraform {
  ...
  backend "gcs" {
    bucket = "licitacerta-staging-tfstate"
...
resource "google_project_service" "apis" {
  for_each = toset([
    "run.googleapis.com", "alloydb.googleapis.com", "storage.googleapis.com",
...
locals {
  secrets = toset(["ANTHROPIC_API_KEY", "DATABASE_URL", "SECRET_KEY"])
}
...
module "storage" {
  ...
  region      = var.region
  bucket_name = "licitacerta-docs-staging"
}
...
module "vpc" {
  ...
  region         = var.region
  network_name   = "licitacerta-staging-vpc"
  depends_on     = [google_project_service.apis]
}

# AlloyDB — ZONAL para staging (sem HA, custo menor)
module "alloydb" {
  source            = "../../modules/alloydb"
  project_id        = var.project_id
  region            = var.region
  cluster_id        = "licitacerta-staging"
  instance_id       = "licitacerta-staging-primary"
...
module "api_service" {
  ...
  project_id            = var.project_id
  region                = var.region
  service_name          = "licitacerta-api-staging"
...
module "worker_service" {
  ...
  project_id            = var.project_id
  region                = var.region
  service_name          = "licitacerta-worker-staging"
...
module "web_service" {
  ...
  project_id            = var.project_id
  region                = var.region
  service_name          = "licitacerta-web-staging"
...
output "api_url"    { value = module.api_service.service_url }
output "worker_url" { value = module.worker_service.service_url }
output "web_url"    { value = module.web_service.service_url }
