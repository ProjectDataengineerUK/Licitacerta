"""Eval: WatchAgent — real LLM calls, incur API cost.

Run with: pytest -m eval tests/evals/test_watch_eval.py
"""
from __future__ import annotations

import pytest

from src.agents.watch import WatchAgent
from tests.evals.conftest import EvalRecorder
from tests.fixtures.watch_evals import EVAL_CASES


@pytest.mark.eval
@pytest.mark.integration
@pytest.mark.parametrize("case", EVAL_CASES, ids=[c["id"] for c in EVAL_CASES])
def test_watch_eval(case: dict, eval_recorder: EvalRecorder) -> None:
    agent = WatchAgent()
    error_msg = None
    result_dict = None
    passed = False
